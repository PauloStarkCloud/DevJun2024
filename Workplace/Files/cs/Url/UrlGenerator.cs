namespace Terrasoft.Configuration.Url
{
	...
	public class UrlGenerator : IUrlGenerator {
		...
		private IEnumerable<PageEntity> GetPageEntities(string schemaName, Guid recordId) {
			...
			var entityPages = PageEntityManager.GetEntityPages(entitySchema.UId).ToList();
			LogInfoObjectList($"Entity pages", entityPages);
			entityPages = entityPages.Where(p => !p.HasSection && !(p.Actions != null && p.Actions.Add)).ToList();
			foreach (var entityPage in entityPages.ToList()) {
				if (entityPage.TypeColumnUId.HasValue) {
					var typeColumn = entitySchema.Columns.FindByUId(entityPage.TypeColumnUId.Value);
					var typeColumnValue = GetEntityTypeColumnValue(entitySchema, recordId, typeColumn.Name);
					if (entityPage.TypeColumnValue != typeColumnValue) {
						LogInfoObject($"Delete entityPages, because type column value does not match", entityPage);
